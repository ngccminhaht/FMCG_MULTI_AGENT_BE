...
export async function requestJson<T>(
  ...
  try {
    ...
    if (!hasExpectedStatus(response.status, options.expectedStatus)) {
      throw new ApiClientError(
        `The API returned unexpected HTTP ${response.status}.`,
        response.status,
    ...
    if (options.validateResponse && !options.validateResponse(parsedBody)) {
      throw new ApiClientError(
        "The API returned an invalid success response. The request was not treated as complete.",
        response.status,
    ...
    if (error instanceof Error && error.name === "AbortError") {
      throw new ApiClientError(
        "The API request timed out. Check the device connection and retry.",
        null,
        "REQUEST_TIMEOUT",
      );
    }
    ...
    throw new ApiClientError(
      "Could not reach the API. Check the device API URL and network connection.",
      null,
      "NETWORK_ERROR",
    );
  } finally {
...
export function formatApiError(error: unknown): string {
  if (!(error instanceof ApiClientError)) {
    return error instanceof Error
      ? error.message
      : "An unexpected client error occurred. Please try again.";
  }
